...
const AdminResetPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${URL}/api/auth/admin-reset/${token}`, {
        password: formData.password,
      });

      if (response.data.success) {
        setSuccess('Password reset successfully. Redirecting to login page...');
        setTimeout(() => navigate('/login-page'), 3000); // Redirect to admin login after 3 seconds
      } else {
        setError(response.data.message || 'Failed to reset password.');
        setTimeout(() => navigate('/register-page'), 3000); // Redirect to admin login after 3 seconds
      }
    } catch (err) {
